    ...
    >
      <swiper-slide v-for="pokemon in pokemonList" :key="pokemon.name">
        ...
        <div class="ml-5">
          ...
          <button @click="showPokemonDetails(pokemon)" class="bg-red-500 shadow-lg transition ease-in-out delay-150  hover:-translate-y-1 hover:scale-110 hover:bg-yellow-500 duration-300 text-white shadow-red-500/50 rounded-md px-2 outline outline-offset-2 outline-red-600 my-5 " type="button">
            Detalles
          </button>
        </div>
      </swiper-slide>
    ...
    <Modal :size="size" v-if="isShowModal" @close="closeModal">
      ...
      <template #body>
        ...
        <div class="text-white font-bold text-xl">
          <ul>
            <li>Indice de felicidad: {{ selectedPokemon.additionalInfo.baseHappiness }} </li>
            <li>Tasa de Captura: {{selectedPokemon.additionalInfo.captureRate}} </li>
            <li>Color: {{selectedPokemon.additionalInfo.color.name}} </li>
            <li>Tipo: {{selectedPokemon.additionalInfo.eggGroups[1].name}} </li>
            <li v-if="selectedPokemon.additionalInfo.evolvesFromSpecies && selectedPokemon.additionalInfo.evolvesFromSpecies.name">
              Pokemon pre-evolucion: {{ selectedPokemon.additionalInfo.evolvesFromSpecies.name }}</li>
              <li v-else>
                Evolucion: Pokemon primario
              </li>
          </ul>
        </div>
      </template>
